...
import { ArrowLeft, UserCheck, MessageSquare, Paperclip, Send, EyeOff, Lock } from 'lucide-react';
...
export default function CaseDetailPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Only secretariat and admin can assign
  ...
  // CM controls are locked once resolved (admin always has full access)
  const cmCanEdit = isCM && !isResolved;
  const cmReadOnly = isCM && isResolved;
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      {/* Resolved lock banner — shown to CM only */}
      {cmReadOnly && (
        <div className="flex items-center gap-3 p-4 mb-6 bg-slate-100 border border-slate-200 rounded-xl text-slate-600">
          <Lock size={18} className="shrink-0 text-slate-400" />
          <div>
            <p className="font-medium text-slate-800">Case Resolved — Read Only</p>
            <p className="text-sm">This case has been resolved and is now locked. No further changes can be made.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-6">
            ...
            <CardContent>
              ...
              {caseData.isAnonymous && (
                <div className="flex items-center gap-1 mt-3 text-xs text-slate-400">
                  <EyeOff size={12} /> Submitted anonymously
                </div>
              )}
            </CardContent>
          </Card>

          {/* Attachments — visible to all */}
            ...
            <Card>
              ...
              <CardContent>
                {caseData.attachments.map((a: any, i: number) => (
                  <a key={i} href={`${process.env.NEXT_PUBLIC_API_URL?.replace('/api', '')}/${a.path}`} target="_blank" rel="noopener noreferrer"
          ...
          {/* Notes — hidden from Secretariat, shown to CM and Admin */}
          {(isCM || isAdmin) && (
            <Card className={cmReadOnly ? 'opacity-60' : ''}>
                      ...
                      {caseData.notes.map((note: any, i: number) => (
                        <div key={i} className={`p-3 rounded-lg border ${note.isInternal ? 'bg-amber-50 border-amber-200' : 'bg-slate-50 border-slate-200'}`}>
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-medium text-slate-700">{note.addedBy?.name || note.authorName || 'System'}</span>
                            {note.isInternal && <Badge variant="warning">Internal</Badge>}
                            <span className="text-xs text-slate-400 ml-auto">{new Date(note.addedAt || note.createdAt).toLocaleString()}</span>
                          </div>
                          <p className="text-sm text-slate-700">{note.text || note.content}</p>
                        </div>
                      ))}
                    </div>
                }

                {/* Add note — only when not resolved (or admin) */}
                {(cmCanEdit || isAdmin) && (
          ...
          {/* Impact & Resolution — hidden from Secretariat, shown to CM and Admin */}
          {(isCM || isAdmin) && (
            <Card className={cmReadOnly ? 'opacity-60' : ''}>
        ...
        <div className="space-y-4">
          ...
          <Card>
            ...
            <CardContent className="space-y-3 text-sm">
              <div>
                ...
                <p className="font-medium">{caseData.isAnonymous ? 'Anonymous' : caseData.submittedBy?.name || '—'}</p>
              </div>
              <Separator />
              <div>
                <p className="text-slate-500">Assigned to</p>
                <p className="font-medium">{caseData.assignedTo?.name || <span className="text-slate-400">Unassigned</span>}</p>
              </div>
              {caseData.assignedAt && (
                <div>
                  <p className="text-slate-500">Assigned on</p>
                  <p className="font-medium">{new Date(caseData.assignedAt).toLocaleDateString()}</p>
                </div>
              )}
              {caseData.resolvedAt && (
                <>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{new Date(caseData.resolvedAt).toLocaleDateString()}</p>
                  </div>
                </>
              )}
            </CardContent>
          ...
          {/* Status update — CM (only when not resolved) + Admin always */}
          {(cmCanEdit || isAdmin) && (
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button className="w-full" size="sm" onClick={handleUpdate} disabled={updating}>
                  Update Status
                </Button>
              </CardContent>
            </Card>
          ...
          {/* Assign — Secretariat and Admin only */}
          {(isSecretariat || isAdmin) && (
            <Card>
              <CardHeader><CardTitle className="text-base flex items-center gap-2"><UserCheck size={16} /> Assign Case</CardTitle></CardHeader>
              <CardContent className="space-y-3">
                <Select value={assignTo} onChange={e => setAssignTo(e.target.value)}>
                  <option value="">Select case manager…</option>
                  {caseManagers.map((cm: any) => (
                    <option key={cm._id} value={cm._id}>{cm.name} ({cm.department})</option>
                  ))}
                </Select>
                <Button className="w-full" size="sm" onClick={handleAssign} disabled={updating || !assignTo}>
                  Assign Case
                </Button>
              </CardContent>
            </Card>
